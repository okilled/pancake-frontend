...
// from the current swap inputs, compute the best trade and return it.
export function useNinanceDerivedSwapInfo(): {
  currencies: { [field in Field]?: Currency }
  currencyBalances: { [field in Field]?: CurrencyAmount }
  parsedAmount: CurrencyAmount | undefined
  v2Trade: Trade | undefined
  ...
  const inputCurrency = useCurrency(tokens.era.address)
  const outputCurrency = useCurrency(tokens.div.address)
  const recipientLookup = useENS(recipient ?? undefined)
  const to: string | null = (recipient === null ? account : recipientLookup.address) ?? null
  ...
  if (balanceIn && parsedAmount && balanceIn.lessThan(parsedAmount)) {
    inputError = t('Insufficient %symbol% balance', { symbol: currencies[Field.INPUT].symbol })
  }
